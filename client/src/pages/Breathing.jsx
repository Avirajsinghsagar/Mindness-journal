...
function Dashboard() {
  ...
  const trendColor = prediction?.trend === "improving" ? "#4CAF50"
    : prediction?.trend === "declining" ? "#f44336" : "#FF9800";
  ...
  const chartData = {
    ...
    datasets: [{
      ...
      label: "Energy",
      data: [...moods].reverse().map(m => m.energy || 0),
      borderColor: "#9b59b6",
  ...
  return (
    <div style={{ padding: "30px", maxWidth: "900px", margin: "0 auto", background: "#f7f9fc", minHeight: "100vh" }}>
      ...
      <p style={{ color: "#888", marginBottom: "24px", fontSize: "14px" }}>
        Track your mood patterns and emotional wellbeing
      </p>
      ...
      {anomaly?.alert && (
        <div style={{
          ...
          padding: "14px 20px", marginBottom: "20px", display: "flex", gap: "12px", alignItems: "center"
        }}>
      ...
      <div style={{ display: "flex", gap: "16px", marginBottom: "24px", flexWrap: "wrap" }}>
        ...
        {prediction && (
          <StatCard emoji="🔮" label="Tomorrow's Mood" value={prediction.predicted_mood}
            color={trendColor} sub={`Trend: ${prediction.trend} • ${prediction.confidence} confidence`} />
        )}
        {anomaly && !anomaly.alert && (
          <StatCard emoji="✅" label="Pattern" value="Normal" color="#4CAF50" sub={anomaly.message} />
        )}
      </div>
        ...
        <div style={{ display: "flex", gap: "32px", flexWrap: "wrap", alignItems: "flex-end" }}>
          <div>
            <label style={{ fontSize: "13px", color: "#666", display: "block", marginBottom: "8px" }}>
              😊 Mood Level: <b style={{ color: "#4a90e2" }}>{mood}/10</b>
            </label>
            <input type="range" min="1" max="10" value={mood}
              onChange={(e) => setMood(Number(e.target.value))}
              style={{ width: "180px", accentColor: "#4a90e2" }} />
          </div>
          <div>
            <label style={{ fontSize: "13px", color: "#666", display: "block", marginBottom: "8px" }}>
              ⚡ Energy Level: <b style={{ color: "#9b59b6" }}>{energy}/10</b>
            </label>
            <input type="range" min="1" max="10" value={energy}
              onChange={(e) => setEnergy(Number(e.target.value))}
              style={{ width: "180px", accentColor: "#9b59b6" }} />
          </div>
          <button onClick={submitMood} disabled={saving} style={{
      ...
      {moods.length === 0 && (
        <div style={{
          ...
          textAlign: "center", color: "#aaa", boxShadow: "0 2px 12px rgba(0,0,0,0.06)"
        }}>
